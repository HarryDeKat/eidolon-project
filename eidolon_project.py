...
def update_rss_feed(history):
    """Generates and overwrites the rss.xml file, using the content:encoded standard for full HTML."""
    print("Stage 3/3: Updating the RSS feed with content:encoded standard...")

    # Define the namespaces we'll use. This is crucial.
    namespaces = {
    ...
    # IMPORTANT: Update this link to your actual GitHub Pages URL.
    link = "https://harrydekat.github.io/eidolon-project/" # Use your actual repo URL
    ET.SubElement(channel, "link").text = link
    # Add the atom:link for self-discovery
    ...
    # --- Style Definitions (kept from previous version) ---
    ...
    for entry in history:
        ...
        ET.SubElement(item, "link").text = link # Link to the main site for each item
        ...
        # --- 1. Create the plain-text description (for old readers) ---
        # Create a short excerpt of the story.
        ...
        # --- 2. Create the full HTML content for modern readers ---
        ...
        """
        # We wrap the final HTML in CDATA to prevent XML parsers from touching it.
        cdata_content = f"<![CDATA[{full_html_content.strip()}]]>"
        
        # Use the QName object to correctly create the <content:encoded> tag
        ET.SubElement(item, ET.QName(namespaces["content"], "encoded")).text = cdata_content

    # --- Final XML Generation ---
    # The tostring method will escape our CDATA wrapper, so we need to decode and unescape it.
    # This is a standard and robust way to handle this with the built-in library.
    ...
    print(f"-> RSS feed successfully updated using <content:encoded> standard.")
