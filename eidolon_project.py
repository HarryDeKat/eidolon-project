...
# eidolon_project.py (the definitive, final version of the function)

def update_rss_feed(history):
    """
    Generates a correctly formatted and robust RSS feed, pointing each item's
    link to its unique URL in the static viewer.html archive.
    """
    print("Stage 3/3: Building final RSS feed with permanent web links...")
    ...
    # This is the base URL for your project's GitHub Pages site.
    ...
    ET.SubElement(channel, "link").text = viewer_url # The main link points to the viewer
    ...
    for entry in history:
        ...
        # --- NEW: Create the permanent link for this specific story ---
        # The ISO date string is a perfect unique ID.
        story_link = f"{viewer_url}?id={entry['date']}"
        
        ET.SubElement(item, "title").text = clean_invalid_xml_chars(entry['title'])
        ET.SubElement(item, "link").text = story_link # The item link now points to the viewer
        pub_date = datetime.datetime.fromisoformat(entry['date']).strftime("%a, %d %b %Y %H:%M:%S %z")
        ET.SubElement(item, "pubDate").text = pub_date
        # The GUID should be a unique, permanent identifier. The new link is perfect for this.
        ...
        # Add a "Read on Web" link at the bottom of the content for convenience
        full_html_content = f"""
    ...
    print("-> RSS feed successfully built with permanent web links.")
